...
setInterval(function () {
    $.ajax({
        ...
        complete: function (data) {
            ...
            if (current_series) {
                $('#promo-now-txt').text(current_series.name_rus === "" ? current_series.name : current_series.name_rus);
                $('#promo-now-img').css('background-image', `url('./static/sources/promos/${current_series.name}.png')`);
            }
            if (next_series) {
                $('#promo-next-txt').text(next_series.name_rus === "" ? next_series.name : next_series.name_rus);
